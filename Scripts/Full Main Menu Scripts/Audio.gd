...
func _ready():
	%Master.value = SaveSettings.config.get_value("Audio", '0')
	AudioServer.set_bus_volume_db(0,linear_to_db(%Master.value))
	
	%Music.value = SaveSettings.config.get_value("Audio", '1')
	AudioServer.set_bus_volume_db(1,linear_to_db(%Music.value))
	
	%SFX.value = SaveSettings.config.get_value("Audio", '2')
...
func set_volume(idx,value):
	...
	SaveSettings.config.set_value("Audio",str(idx),value)
